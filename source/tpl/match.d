...
public:
	...
	override bool found() { return !empty; }
	...
	override size_t position()  { return offset_+prefix.length; }
	override size_t end_position()  { return position() + str(0).length; }
...
public:
	...
	size_t position()  { return _open_match.position(); }
	size_t end_position()  { return _close_match.end_position(); }
	size_t length()  { return _close_match.end_position() - _open_match.position(); }
	bool found()  { return _open_match.found() && _close_match.found(); }
	string prefix()  { return _open_match.prefix(); }
	string suffix()  { return _close_match.suffix(); }
	string outer()  { return _open_match.str(0) ~ _open_match.suffix()[0.._close_match.end_position() - _open_match.end_position()]; }
	string inner()  { 
			writeln("close pos : ",_close_match.position(),"    open end : ",_open_match.end_position());
...
public:
	...
	static MatchClosed search_closed_on_level(string input, string regex_statement,  string regex_level_up, string regex_level_down, string regex_search, Match open_match) {
	...
	static MatchClosed search_closed(string input, string regex_statement, string regex_open, string regex_close, Match open_match) {
