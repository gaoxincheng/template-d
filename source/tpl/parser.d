	...
	ElementExpression parse_expression(const string input) {
		...
		switch ( match_function.type() ) {
			...
			default: {
				...
				for ( int i = 1; i < match_function.size(); i++) { // str(0) is whole group
			...
			switch ( match_delimiter.type() ) {
				...
				case Delimiter.LineStatement: {
					...
					switch ( match_statement.type() ) {
						case Statement.Loop: {
							MatchClosed loop_match = RegexObj.search_closed(input, match_delimiter.pattern(), regex_map_statement_openers[Statement.Loop], regex_map_statement_closers[Statement.Loop], match_delimiter);
							...
							switch (match_command.type()) {
								case Loop.ForListIn: {
									...
									result ~= new ElementLoop(match_command.type(), value_name, parse_expression(list_name), loop_match.inner());
									break;
								}
								case Loop.ForMapIn: {
									...
									result ~= new ElementLoop(match_command.type(), key_name, value_name, parse_expression(list_name), loop_match.inner());
									break;
								}
                                default:
                                    inja_throw("parser_error", "unknown loop statement: " ~ match_command.str());
                                    break;
							}
							break;
						}
						case Statement.Condition: {
							...
							MatchClosed else_if_match = RegexObj.search_closed_on_level(input, match_delimiter.pattern(), regex_map_statement_openers[Statement.Condition], regex_map_statement_closers[Statement.Condition], regex_map_condition[Condition.ElseIf], condition_match);
							while (else_if_match.found()) {
								...
								auto match_command = RegexObj.match!Condition(else_if_match_inner, regex_map_condition);
								if (!match_command.found()) {
									inja_throw("parser_error", "unknown if statement: " ~ else_if_match._open_match.str());
								}
								condition_container.children ~= new ElementConditionBranch(else_if_match.inner(), match_command.type(), parse_expression(match_command.str(1)));

								else_if_match = RegexObj.search_closed_on_level(input, match_delimiter.pattern(), regex_map_statement_openers[Statement.Condition], regex_map_statement_closers[Statement.Condition], regex_map_condition[Condition.ElseIf], condition_match);
							}

							MatchClosed else_match = RegexObj.search_closed_on_level(input, match_delimiter.pattern(), regex_map_statement_openers[Statement.Condition], regex_map_statement_closers[Statement.Condition], regex_map_condition[Condition.Else], condition_match);
							if (else_match.found()) {
								condition_match = else_match._close_match;

								string else_match_inner = else_match._open_match.str(1);
								auto match_command = RegexObj.match!Condition(else_match_inner, regex_map_condition);
								if (!match_command.found()) {
                           ...
                           //MatchClosed last_if_match = RegexObj.search_closed_on_level(input, match_delimiter.pattern(), regex_map_statement_openers[Statement.Condition], regex_map_statement_closers[Statement.Condition], regex_map_statement_closers[Statement.Condition], condition_match);
							...
							auto match_command = RegexObj.match!Condition(last_if_match_inner, regex_map_condition);
							if (!match_command.found()) {
                        ...
                        default:{
                            inja_throw("parser_error", "unknown  statement: " ~ to!string(match_statement.type()));
                            break;
                        }
					}
					break;
                ...
                default:{
                    inja_throw("parser_error", "unknown  statement: " ~ to!string(match_delimiter.type()));
                    break;
                }
			}
